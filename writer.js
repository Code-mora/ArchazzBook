...
function initializePageEditor(pageId, initialContent) {
  ...
  // IMPORTANT: Load initial content FIRST before attaching any listeners
  // This prevents text-change from firing and overwriting page.content with empty string
  if (initialContent) {
    // Use dangerouslyPasteHTML which is more reliable than setting innerHTML directly
    quill.clipboard.dangerouslyPasteHTML(0, initialContent);
  }

  // Set default font AFTER content is loaded (quill.format fires text-change)
  quill.format('font', 'times-new-roman');

  // Store editor instance
  pageEditors.set(pageId, quill);

  // NOW attach text-change listener - after initial content is already settled
