...
function newChapter() {
    ...
    const chapterTitle = prompt(`Enter title for Chapter ${chapterNumber}:`, `Chapter ${chapterNumber}`);
...
function createPageElement(page, pageNumber, totalPages) {
    ...
    pageWrapper.innerHTML = `
        <div class="page-header-info">
            ...
            ${totalPages > 1 ? `<button class="page-delete-btn" onclick="deletePage(${page.id})" title="Delete page">
                <i class="fas fa-trash"></i>
            </button>` : ''}
        </div>
...
function initializePageEditor(pageId, initialContent) {
    ...
    const autoGeneratedToolbar = document.querySelector(`#editor-container-${pageId} .ql-toolbar`);
    ...
    if (wordCount > 500) {
        ...
        if (pageEl && !pageEl.dataset.notified) {
            ...
            indicator.style.cssText = `
                ...
                cursor: pointer;
            `;
            indicator.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Page getting full. <strong>Add new page?</strong>';
...
function updateStats() {
    ...
    document.getElementById('total-chapters').textContent = currentBook.chapters.length;
}
